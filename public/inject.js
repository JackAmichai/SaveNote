 ...
 * Pixel-Perfect Native Bot Mode without Extension - FIXED v3
 */
(function () {
    'use strict';
    ...
    var BOT_COLOR = '#008069';    function categorize(t) {
    ...
    // SVG for bot avatar
    var BOT_SVG = '<svg viewBox="0 0 24 24" width="100%" height="100%"><circle cx="12" cy="12" r="12" fill="' + BOT_COLOR + '"/><text x="12" y="17" text-anchor="middle" fill="white" font-size="14" font-weight="bold">S</text></svg>';

    // SVG tail for bot message
    var TAIL_SVG = '<svg viewBox="0 0 8 13" width="8" height="13"><path opacity=".13" fill="#000000" d="M1.533 3.568 8 12.193V1H2.812C1.042 0 .474 1.156 1.533 2.568z"/></svg>';
    ...
    function injectBotReply(html) {
        try {
            ...
            row.innerHTML = '<div class="sn-bot-bubble-wrapper">' +
                            ...
                            '<div class="sn-bot-meta"><span class="sn-bot-time">' + nowStr + '</span></div>' +
                            '</div></div>';
    ...
    function handleCommand(text) {
        ...
        if (lower.indexOf('what') !== -1 && lower.indexOf('book') !== -1) {
            ...
            setTimeout(function() {
                if (books.length === 0) {
                    injectBotReply('\uD83D\uDCDA You haven\'t saved any books yet.');
                } else {
        ...
        if (lower.indexOf('where') !== -1 && lower.indexOf('park') !== -1) {
            ...
            setTimeout(function() {
                if (!p) {
                    injectBotReply('\uD83C\uDD70\uFE0F No parking spot found.');
                } else {
                    injectBotReply('\uD83C\uDD70\uFE0F Last parking spot:  \n"' + p.raw_message + '"');
                }
            }, 1500);
            return true;
        }
        ...
        if (lower.indexOf('what') !== -1 && lower.indexOf('shopping') !== -1) {
            ...
            setTimeout(function() {
                if (shoppingList.length === 0) {
                    injectBotReply('\uD83D\uDED2 Shopping list empty.');
                } else {
        ...
        if (lower.indexOf('what') !== -1 && lower.indexOf('note') !== -1) {
            ...
            setTimeout(function() {
                if (all.length === 0) {
                    injectBotReply('\uD83D\uDCC4 No notes saved yet.');
                } else {
